...
if [[ ":$PATH:" != *":$INSTALL_DIR/git:"* ]]; then
    ...
    echo 'export PATH="$INSTALL_DIR/git:$PATH"' >> ~/.zshrc  # Adjust for your shell (e.g., ~/.bashrc for Bash)
fi

if [[ ":$PATH:" != *":$INSTALL_DIR/conventional:"* ]]; then
    echo "Adding $INSTALL_DIR/conventional to the PATH..."
    echo 'export PATH="$INSTALL_DIR/conventional:$PATH"' >> ~/.zshrc  # Adjust for your shell
fi

if [[ ":$PATH:" != *":$INSTALL_DIR/docker:"* ]]; then
    echo "Adding $INSTALL_DIR/docker to the PATH..."
    echo 'export PATH="$INSTALL_DIR/docker:$PATH"' >> ~/.zshrc  # Adjust for your shell
fi

# Reload the shell configuration to apply changes
source ~/.zshrc  # Adjust for your shell
